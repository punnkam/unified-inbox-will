...
export default function GeneralContent({
  ...
  workspaceId: string;
  generalSettings: Workspace["generalSettings"] | undefined;
}) {
  const [imagePreview, setImagePreview] = useState<
    Workspace["generalSettings"]["logo"]
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    handleChange("logo", file?.name || undefined);
  };

  const initialData: Workspace["generalSettings"] = generalSettings || {
    name: "",
  };

  const saveData = async (data: Workspace["generalSettings"]) => {
    // Call API to save the data
    // console.log("Saving data", data);

    const response = await saveGeneralSettings(workspaceId, data);
