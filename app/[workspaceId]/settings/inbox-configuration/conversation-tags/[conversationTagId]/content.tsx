...
import { deleteConversationTag, saveConversationTag } from "@/app/actions";
...
export default function EditConversationTagContent({
  conversationTag,
}: {
  conversationTag: ConversationTag;
}) {
  const [data, setData] = useState<ConversationTag>(conversationTag);
  ...
  const handleSave = async () => {
    ...
    if (response.success) {
      toast.success("Conversation tag saved successfully");

      // After uploading the new tag - route back to main page
      router.push("../conversation-tags/");
    } else {
      toast.error("Error saving conversation tag: " + response.message);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    // Call your API here
    const response = await deleteConversationTag(data);
  ...
  return (
    <div className="flex flex-col gap-[28px]">
      <div className="flex flex-col gap-4">
        ...
        <h1 className="text-title-2xl">Edit conversation tag</h1>
      </div>
      ...
      <div className="flex justify-between items-center">
        <AlertDialog open={isDialogOpen}>
          ...
          >
            {loading.delete ? "Deleting tag..." : "Delete tag"}
          </Button>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <Button
            ...
            onClick={() => handleSave()}
            disabled={
              loading.save || data.name === "" || data.description === ""
            }
          >
